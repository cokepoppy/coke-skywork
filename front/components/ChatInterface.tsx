...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const loadedPPTIdRef = useRef<string | null>(null); // Track loaded PPT to avoid re-loading same PPT
  ...
  useEffect(() => {
    console.log('[ChatInterface] useEffect triggered, pptToLoad:', pptToLoad);

    if (pptToLoad && pptToLoad.id !== loadedPPTIdRef.current) {
      console.log('[ChatInterface] Loading PPT from history - ID:', pptToLoad.id);
      loadedPPTIdRef.current = pptToLoad.id; // Mark as loaded
      console.log('[ChatInterface] pptToLoad type:', typeof pptToLoad);
      console.log('[ChatInterface] pptToLoad keys:', Object.keys(pptToLoad));

      try {
        console.log('[ChatInterface] PPT details:', {
          id: pptToLoad.id,
          topic: pptToLoad.topic,
          hasSlideDeck: !!pptToLoad.slideDeck,
          hasGeneratedImage: !!pptToLoad.slideDeck?.generatedImage,
          hasHtmlContent: !!pptToLoad.slideDeck?.htmlContent,
        });
      } catch (e) {
        console.error('[ChatInterface] Error logging PPT details:', e);
      }

      console.log('[ChatInterface] Full pptToLoad object:', pptToLoad);
      ...
      console.log('[ChatInterface] Created loadedDeck:', loadedDeck);
      console.log('[ChatInterface] Checking for HTML mode:', {
        hasHtmlContent: !!loadedDeck.htmlContent,
        htmlContentType: typeof loadedDeck.htmlContent,
        htmlContentLength: loadedDeck.htmlContent?.length,
        htmlContentPreview: loadedDeck.htmlContent?.substring(0, 100),
        hasIsHtmlMode: !!loadedDeck.isHtmlMode,
        generatedImagePrefix: loadedDeck.generatedImage?.substring(0, 50),
      });

      // Fix HTML mode PPTs loaded from history
      // Check both isHtmlMode flag and presence of htmlContent
      if ((loadedDeck.isHtmlMode || loadedDeck.htmlContent) && typeof loadedDeck.htmlContent === 'string') {
        console.log('[ChatInterface] Detected HTML mode PPT from history');

        // Extract clean HTML from htmlContent (remove stream metadata if present)
        let cleanHTML = loadedDeck.htmlContent;

        // Remove stream metadata like "data: {...}" lines
        if (cleanHTML.includes('data: {')) {
          const lines = cleanHTML.split('\n');
          const dataLines = lines.filter(line => line.startsWith('data: {'));
          console.log('[ChatInterface] Found', dataLines.length, 'data lines');
          if (dataLines.length > 0) {
            // Combine all text chunks from streaming response
            let combinedText = '';
            for (const line of dataLines) {
              try {
                ...
                console.warn('[ChatInterface] Failed to parse stream line:', e);
              }
            }
            if (combinedText) {
              cleanHTML = combinedText;
              console.log('[ChatInterface] Combined HTML from', dataLines.length, 'stream chunks, total length:', cleanHTML.length);
              console.log('[ChatInterface] cleanHTML preview:', cleanHTML.substring(0, 200));
            }
          }
        }

        // Extract HTML from markdown code blocks if present
        console.log('[ChatInterface] Attempting to extract from markdown, cleanHTML length:', cleanHTML.length);
        console.log('[ChatInterface] cleanHTML starts with:', cleanHTML.substring(0, 100));

        // Handle both actual newlines and escaped newlines (\n in string)
        let htmlMatch = cleanHTML.match(/```html\n([\s\S]*?)\n```/) || cleanHTML.match(/```\n([\s\S]*?)\n```/);
        console.log('[ChatInterface] First regex match:', !!htmlMatch);

        if (!htmlMatch) {
          // Try with escaped newlines (for JSON-encoded strings)
          htmlMatch = cleanHTML.match(/```html\\n([\s\S]*?)\\n```/) || cleanHTML.match(/```\\n([\s\S]*?)\\n```/);
          console.log('[ChatInterface] Second regex match (escaped):', !!htmlMatch);
        }

        if (htmlMatch) {
          console.log('[ChatInterface] Match found! Matched text length:', htmlMatch[1].length);
          cleanHTML = htmlMatch[1];
          // Unescape newlines if they were escaped
          if (cleanHTML.includes('\\n')) {
            console.log('[ChatInterface] Unescaping newlines...');
            cleanHTML = cleanHTML.replace(/\\n/g, '\n').replace(/\\"/g, '"').replace(/\\\\/g, '\\');
          }
          console.log('[ChatInterface] Extracted HTML from code block, final length:', cleanHTML.length);
        } else {
          console.warn('[ChatInterface] No markdown code block found! Using raw cleanHTML');
        }
        ...
        loadedDeck.generatedImage = htmlBlobUrl;
        loadedDeck.htmlContent = cleanHTML;
        loadedDeck.isHtmlMode = true;

        console.log('[ChatInterface] Fixed HTML mode PPT:', {
          blobUrl: htmlBlobUrl,
      ...
      console.log('[ChatInterface] slideDeck state set');

      console.log('[ChatInterface] Setting isPPTMode to true');
      setIsPPTMode(true);
      console.log('[ChatInterface] isPPTMode state set to true');
      console.log('[ChatInterface] PPT mode activated, should show PPT editor now');
      ...
      // DON'T clear pptToLoad immediately - it will be cleared on next load or when component unmounts
      // Clearing it causes App to re-render which triggers ChatInterface to remount (due to key prop), resetting all state
      console.log('[ChatInterface] PPT load complete (NOT clearing pptToLoad to avoid re-mount)');
    }
  }, [pptToLoad]); // Removed onClearPptToLoad from dependencies to prevent infinite loop
  ...
  const handleSubmit = async () => {
    ...
    if (activeAgent === 'Slides') {
        console.log('[PPT Generation] Starting PPT generation, setting isPPTMode to true');
        setIsPPTMode(true);
        console.log('[PPT Generation] isPPTMode set to true, setting isLoading to true');
        setIsLoading(true);
        ...
        try {
            ...
            if (useNanoBananaPro) {
                // === NANO BANANA PRO MODE: Image-based PPT ===
                console.log('[PPT] Using Nano Banana Pro - Image Generation Mode');
                ...
                setMessages(prev => [...prev, { role: 'model', content: 'Analyzing requirements and generating slide visual with Nano Banana Pro...' }]);
                ...
                console.log('[PPT] Received base64Image:', {
                    length: base64Image?.length,
                    prefix: base64Image?.substring(0, 50),
                    isValidDataUrl: base64Image?.startsWith('data:'),
                });

                // Extract title from content (limit to 200 chars)
                const extractTitle = (content: string): string => {
                  // Try to extract from first line or first few words
                ...
                console.log('[PPT] slideDeck state updated (Image mode), should show PPT preview now');

                // Save to history
                console.log('[PPT] Saving to history (Image mode)...');
                onSavePPTToHistory(newSlideDeck);
                console.log('[PPT] Save to history complete (Image mode)');

                // Update the bot message to confirm
                setMessages(prev => {
                    const newMsgs = [...prev];
                    newMsgs[newMsgs.length - 1].content = `I've generated a presentation slide for "**${userMessage.content}**" using the ${selectedStyle.title} style with Nano Banana Pro. You can preview and download it on the right.`;
                    return newMsgs;
                });

            } else {
                // === GEMINI PRO 3 MODE: HTML-based PPT ===
                console.log('[PPT] Using Gemini Pro 3 - HTML Generation Mode');

                // Mock a "thinking" message from the bot
                setMessages(prev => [...prev, { role: 'model', content: 'Generating HTML presentation with Gemini Pro 3...' }]);
                ...
                console.log('[PPT] Created HTML Blob URL:', {
                    blobUrl: htmlBlobUrl,
                ...
                setSlideDeck(newSlideDeck);
                console.log('[PPT] slideDeck state updated (HTML mode), should show PPT preview now');
                console.log('[PPT] Current isPPTMode should be true');

                // Save to history
                console.log('[PPT] Saving to history (HTML mode)...');
                onSavePPTToHistory(newSlideDeck);
                console.log('[PPT] Save to history complete (HTML mode)');

                // Update the bot message to confirm
                setMessages(prev => {
                    const newMsgs = [...prev];
                    newMsgs[newMsgs.length - 1].content = `I've generated an HTML presentation for "**${userMessage.content}**" using the ${selectedStyle.title} style with Gemini Pro 3. You can preview it on the right.`;
                    return newMsgs;
                });
            }
            ...
            let errorMessage = `Failed to generate ${useNanoBananaPro ? 'slide image' : 'HTML presentation'}: ${error.message || 'Unknown error'}`;
            ...
            console.log('[PPT Generation] PPT generation complete, setting isLoading to false');
            console.log('[PPT Generation] isPPTMode should remain true, current slideDeck exists:', !!slideDeck);
            setIsLoading(false);
            console.log('[PPT Generation] isLoading set to false, PPT mode should be visible');
        }
        return;
    }
  ...
  console.log('[ChatInterface RENDER] Current state:', {
    isPPTMode,
    hasSlideDeck: !!slideDeck,
    slidedeckTopic: slideDeck?.topic,
    messagesCount: messages.length
  });

  if (isPPTMode) {
    console.log('[ChatInterface RENDER] Entering PPT mode, rendering PPT editor');
  } else {
    console.log('[ChatInterface RENDER] In normal chat mode, NOT showing PPT editor');
  }
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-6">
                    {messages.length === 0 && (
                        <div className="text-skywork-muted text-sm p-4 bg-skywork-surface rounded-lg">
                            Hello, I'm Coke Agent. I'm ready to create a slide deck for you! You're now in Slide Agent mode using Gemini 3 Pro Image Preview model. Please send your slide requirements.
                        </div>
                    )}
            ...
            <div className="flex-1 flex flex-col bg-[#050505] text-white overflow-hidden relative border-l border-skywork-border">
                ...
                <div className="h-14 bg-skywork-surface border-b border-skywork-border flex items-center justify-between px-4 shadow-sm z-10">
                   <div className="flex items-center gap-4 text-skywork-muted">
                      ...
                      <span className="text-xs text-skywork-muted">
                         {useNanoBananaPro ? 'Nano Banana Pro (Image)' : 'Gemini Pro 3 (HTML)'}
                      </span>
                   </div>
    ...
    >
      ...
      {!isLanding && (
        <header className="h-16 flex items-center justify-between px-6 border-b border-skywork-border/50 backdrop-blur-md bg-skywork-bg/80 sticky top-0 z-10">
          <div className="flex items-center gap-3">
             <span className="font-medium text-skywork-text">Coke Agent Chat</span>
        ...
        {isLanding ? (
          <div className="h-full flex flex-col items-center justify-center pt-[10vh] animate-fade-in w-full max-w-[860px] mx-auto">
            ...
            <div className="mb-4 flex items-center gap-2 px-3 py-1 rounded-full bg-skywork-surface border border-skywork-border/50">
                <span className="text-sm font-medium text-skywork-text">Coke Agent Super Agents</span>
                ...
                <div className="bg-skywork-surface border border-skywork-border rounded-[20px] shadow-2xl p-4 min-h-[160px] flex flex-col justify-between relative overflow-hidden group hover:border-skywork-border/80 transition-colors">
                    ...
                    <div className="flex items-center justify-between mt-4">
                        ...
                        <div className="flex items-center gap-2">
                             ...
                             <button
                                onClick={() => setUseNanoBananaPro(!useNanoBananaPro)}
                                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl transition-colors text-xs font-medium border ${
                                   useNanoBananaPro
                                      ? 'bg-[#FFF8E6] hover:bg-[#FFF0CC] text-[#522712] border-[#FBE851]'
                                      : 'bg-skywork-surface hover:bg-skywork-surface/70 text-skywork-muted border-skywork-border'
                                }`}
                             >
                                <span className="text-lg leading-none">🍌</span>
                                <span>Nano Banana Pro</span>
                                {useNanoBananaPro && (
                                   <span className="ml-1 px-1 py-0.5 bg-[#EEE0FF] text-[#7826FF] text-[9px] rounded uppercase tracking-wide">Active</span>
                                )}
                             </button>
            ...
            <div className={`relative bg-skywork-surface/90 backdrop-blur-xl border border-skywork-border rounded-[20px] shadow-2xl transition-all duration-300 ${isLoading ? 'opacity-80' : 'opacity-100'}`}>
                ...
                <div className="flex flex-col p-4">
                   <textarea
                    ...
                    onChange={(e) => setInput(e.target.value)}
                    onKeyDown={handleKeyDown}
                    placeholder="Message Coke Agent..."
